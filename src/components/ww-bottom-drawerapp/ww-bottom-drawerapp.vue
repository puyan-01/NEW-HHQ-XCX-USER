...
<template>
  <view>
    ...
    <view
      ...
      :data-top="dragTop"
      :style="transition + 'bottom:' + (dragTop + 'px')"
    >
      <view
        id="drag-content"
        class="drag-content"
      ...
      >
        <view
          class="drag-handle drag-handle-ali"
...
export default {
  ...
  methods: {
    initTop() {
      ...
      query
        ...
        .boundingClientRect((data) => {
          ...
          this.transition = ''
          this.dragTop = -Number(
            (1 - this.proportionShow) * this.contentHeight,
          )
    ...
    drag_start(event) {
      // console.log(this.canDrag); //返回true
      if (!this.canDrag) {
        return
      }
      if (this.timer) {
    ...
    drag_move(event) {
      ...
      dom.getComponentRect(
        this.$refs['drag-content'],
        (ret) => {
          let len = event.touches[0].screenY - this.startY
          ...
          let temp_ = Number(temp)
          let ss_ = -Number(
            (1 - this.proportionMiniShow) *
              this.contentHeight,
          )
          if (Number(temp_) < ss_) {
            this.dragTop = ss_
      ...
      query
        .select('#drag-content')
        .boundingClientRect((data) => {
          let len = event.touches[0].clientY - this.startY
          //判断展开还是收缩
          if (this.isDrawerOpen) {
            //当展开状态下
            if (len > 0) {
    ...
    animationTop(from, to, callback) {
      ...
      let interval = 20
      // 定义总次数
      let allCount = time / interval

      // 定义步长
      let step = (to - from) / allCount
